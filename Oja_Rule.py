...
def learn_oja(input_data, initial_weights=None, learning_rate = 0.005):
    ...
    if initial_weights is None:
        initial_weights = 2* np.random.random_sample(n) - 1 # random values between -1,1
    initial_weights = initial_weights / np.sqrt(np.sum(np.square(initial_weights))) # rescale the initial weights to squared sum 1
...
def create_input_data(num_points, num_dimensions = 2, stds=None):
    ...
    if num_points/num_dimensions <= 10:
        Warning('The ration of num_points to num_dimensions is low, should be at least 10')
...
def plot_oja(input_data, weights, y, do_dataPlot = 'false'):
    """
    ...
    if do_dataPlot:
        if np.size(input_data,1) != 2:
            ...
            sm = plt.cm.ScalarMappable(
                cmap=plt.cm.cool,
                norm=plt.Normalize(vmin=0, vmax=len(input_data))
            )
            sm.set_array(time)
            cb = plt.colorbar(sm)
...
def run_oja(num_points, num_dimensions = 2, stds=None, initial_weights=None, learning_rate = 0.005,
            do_dataPlot = 'false'):
